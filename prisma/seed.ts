...
async function main() {
  ...
  await prisma.supplementTags.deleteMany();
  ...
  await prisma.postalCodeRegion.deleteMany();
  ...
  await prisma.affiliate.createMany({
    data: [
      {
        code: 'test_affiliate_1',
        rewardType: 'FREE_MONTH',
        rewardAmount: 6,
        description: '6 months free subscription for using affiliate link',
      },
      {
        code: 'test_affiliate_2',
        rewardType: 'CREDIT',
        rewardAmount: 10,
        description: '10% discount credit for using affiliate link',
      },
    ],
  });
  ...
  const supTP1 = await prisma.supplementTeamProducts.findFirst({
    where: {
      status: 'ACTIVE',
    },
  });
  const supTP2 = await prisma.supplementTeamProducts.findFirst({
    where: {
      status: 'PREORDER',
    },
  });
  ...
  await prisma.order.createMany({
    data: [
      {
        teamId: supTP1.teamId,
        deadline: new Date(Date.now() + 1000 * 60 * 60 * 24 * 60),
        deliveryDate: new Date(Date.now() + 1000 * 60 * 60 * 24 * 62),
      },
      {
        teamId: supTP2.teamId,
        deadline: new Date(Date.now() + 1000 * 60 * 60 * 24 * 60),
        deliveryDate: new Date(Date.now() + 1000 * 60 * 60 * 24 * 65),
        firstDelivery: true,
      },
    ],
  });
